...
monitor_ptr = 0
monitor_sample = [0] * 100
monitor_print = 0

monitor_txt = 1

#=======================================================================
# Use CPU3 (among 4 cores of 0~3)
#=======================================================================
'''
def cpu3():
    try:
        ...
        print("Error occured during set CPU 3:", e)
cpu3()
'''
...
#---------------set common reg
gv.MIC_SEL = 0                          #Mic. selection 0~111
gv.TEST_ENABLE = 1                      #0=disable(real mic signal), 1=enable (TEST_FREQ applied)
...
gv.spi0.max_speed_hz = 24995000  #24MHz
...
# GPIO numbers of Main board
gpio_sleep_enable =  7
gpio_rom_update_enable =  14
gpio_new_bf_data =  16
gpio_spi_reset_n = 17
...
def gpio_interrupt_handler(channel):
    ...
    global monitor_sample, monitor_ptr, monitor_print
    ...
    if gv.cnt >= 25 :        
        ...
        checksum_str = str(csum_check)

        checksum_result = "checksum: " + checksum_str + "\n"

        f = open("log.txt", "a")
        f.write(checksum_result)
        f.close()
    ...
    monitor_sample[monitor_ptr] = lapse_ms
    ...
    lapse_ms_sum = sum(monitor_sample)
    lapse_ms_avg = lapse_ms_sum / 100

    lapse_ms_max = 0
    lapse_ms_min = 100

    for n in monitor_sample:
    ...
    if monitor_print  == 1:
        ...
        date_time_str = t_now.strftime("%Y-%m-%d %H:%M:%S")

        txt ="[ " +date_time_str + " ] " + "py last 100: max: {0}, avg: {1}, min: {2} ms\n".format(lapse_ms_max, lapse_ms_avg, lapse_ms_min)
